...
check_and_set_port() {
    if lsof -i:"${LISTEN_PORT}" &>/dev/null; then
        ...
        while true; do read -r -p "请输入一个新的可用端口: " NEW_PORT; if [[ "$NEW_PORT" =~ ^[0-9]+$ ]] && [ "$NEW_PORT" -gt 0 ] && [ "$NEW_PORT" -lt 65536 ] && ! lsof -i:"${NEW_PORT}" &>/dev/null; then LISTEN_PORT=${NEW_PORT}; log_success "将使用新端口: ${LISTEN_PORT}"; break; else log_warning "无效或已被占用的端口。"; fi; done
    else log_info "默认端口 ${LISTEN_PORT} 可用。"; fi
}
prompt_for_sni() {
    log_info "您可以指定一个 SNI 用于伪装流量。"; read -r -p "请输入 SNI (例如 www.apple.com)，或直接按回车随机选择: " USER_SNI
    if [ -z "$USER_SNI" ]; then RANDOM_INDEX=$((RANDOM % ${#SNI_LIST[@]})); SELECTED_SNI=${SNI_LIST[$RANDOM_INDEX]}; log_success "已随机选择 SNI: ${SELECTED_SNI}"; else SELECTED_SNI=$USER_SNI; log_success "将使用您指定的 SNI: ${SELECTED_SNI}"; fi
}
install_bbr() {
    log_info "检查并安装 BBR..."; if ! sysctl -n net.ipv4.tcp_congestion_control | grep -q "bbr"; then echo "net.core.default_qdisc=fq" >> /etc/sysctl.conf; echo "net.ipv4.tcp_congestion_control=bbr" >> /etc/sysctl.conf; sysctl -p &>/dev/null; fi
    if sysctl -n net.ipv4.tcp_congestion_control | grep -q "bbr"; then log_success "TCP 拥塞控制算法已设置为 bbr。"; else log_warning "BBR 启用失败。"; fi
    if sysctl -n net.core.default_qdisc | grep -q "fq"; then log_success "队列调度算法已设置为 fq。"; else log_warning "队列调度算法设置 fq 失败。"; fi
}
check_bbr_status() {
    clear; log_info "正在检查 BBR 和 FQ 状态..."; sleep 1; local bbr_status=$(sysctl -n net.ipv4.tcp_congestion_control); local qdisc_status=$(sysctl -n net.core.default_qdisc)
    echo -e "================ BBR + FQ 状态检查 ================"
    if [[ "$bbr_status" == "bbr" ]]; then echo -e "  ${GREEN}✅ TCP 拥塞控制算法: bbr (已启用)${NC}"; else echo -e "  ${RED}❌ TCP 拥塞控制算法: ${bbr_status} (BBR 未启用)${NC}"; fi
    if [[ "$qdisc_status" == "fq" ]]; then echo -e "  ${GREEN}✅ 默认队列调度算法: fq (已启用)${NC}"; else echo -e "  ${YELLOW}⚠️  默认队列调度算法: ${qdisc_status} (建议使用 fq)${NC}"; fi
    echo -e "=================================================="; read -n 1 -s -r -p $'\n按任意键返回主菜单...';
}
generate_config() {
    log_info "正在生成配置文件..."; mkdir -p "${SINGBOX_CONFIG_PATH}"; local UUID=$(${SINGBOX_BINARY_PATH} generate uuid); local KEY_PAIR=$(${SINGBOX_BINARY_PATH} generate reality-keypair); local PRIVATE_KEY=$(echo "${KEY_PAIR}" | awk '/PrivateKey/ {print $2}' | tr -d '"'); local PUBLIC_KEY=$(echo "${KEY_PAIR}" | awk '/PublicKey/ {print $2}' | tr -d '"'); local SHORT_ID=$(openssl rand -hex 8);
    cat > "${SINGBOX_CONFIG_FILE}" <<EOF
{ "log": {"level": "info", "timestamp": true}, "x_public_key": "${PUBLIC_KEY}", "inbounds": [{"type": "vless", "tag": "vless-in", "listen": "::", "listen_port": ${LISTEN_PORT}, "sniff": true, "sniff_override_destination": true, "users": [{"uuid": "${UUID}", "flow": "xtls-rprx-vision"}], "tls": { "enabled": true, "server_name": "${SELECTED_SNI}", "reality": { "enabled": true, "handshake": {"server": "${SELECTED_SNI}", "server_port": 443}, "private_key": "${PRIVATE_KEY}", "short_id": ["${SHORT_ID}"] } } }], "outbounds": [{"type": "direct", "tag": "direct"}, {"type": "block", "tag": "block"}] }
EOF
}
create_service() {
     log_info "正在创建 systemd 服务..."; cat > "${SINGBOX_SERVICE_FILE}" <<EOF
[Unit]
...
uninstall_sing-box() {
    log_warning "确定要卸载 sing-box 吗? [y/N]"; read -r -p "请输入: " confirm; [[ ! "$confirm" =~ ^[yY]$ ]] && log_info "卸载操作已取消。" && return; systemctl stop sing-box; systemctl disable sing-box; rm -f "${SINGBOX_SERVICE_FILE}" "${SINGBOX_BINARY_PATH}" "${SCRIPT_PATH}"; rm -rf "${SINGBOX_CONFIG_PATH}"; if grep -q "alias ${SHORTCUT_NAME}=" ~/.bashrc; then sed -i "/alias ${SHORTCUT_NAME}=/d" ~/.bashrc; log_info "已移除快捷命令。"; fi; systemctl daemon-reload; log_success "sing-box 已成功卸载。"
}
display_node_info() {
    [ ! -f "${SINGBOX_CONFIG_FILE}" ] && log_error "配置文件不存在。" && return; CFG_PORT=$(jq -r '.inbounds[0].listen_port' "${SINGBOX_CONFIG_FILE}"); UUID=$(jq -r '.inbounds[0].users[0].uuid' "${SINGBOX_CONFIG_FILE}"); PUBLIC_KEY=$(jq -r '.x_public_key' "${SINGBOX_CONFIG_FILE}"); SHORT_ID=$(jq -r '.inbounds[0].tls.reality.short_id[0]' "${SINGBOX_CONFIG_FILE}"); SERVER_NAME=$(jq -r '.inbounds[0].tls.server_name' "${SINGBOX_CONFIG_FILE}"); IP_ADDR=$(curl -s4 ip.sb || curl -s4 ifconfig.me); NODE_NAME="vps-$(date +%s)"; [ -z "$PUBLIC_KEY" ] || [ "$PUBLIC_KEY" == "null" ] && log_error "无法读取公钥，请卸载后重装。" && return; VLESS_LINK="vless://${UUID}@${IP_ADDR}:${CFG_PORT}?encryption=none&flow=xtls-rprx-vision&security=reality&sni=${SERVER_NAME}&fp=chrome&pbk=${PUBLIC_KEY}&sid=${SHORT_ID}&type=tcp#${NODE_NAME}";
    clear ; echo -e "================ 节点配置信息 ================"; echo -e "  地址 (Address): ${IP_ADDR}\n  端口 (Port): ${CFG_PORT}\n  UUID: ${UUID}\n  流控 (Flow): xtls-rprx-vision\n  安全 (Security): reality\n  SNI: ${SERVER_NAME}\n  公钥 (pbk): ${PUBLIC_KEY}\n  Short ID (sid): ${SHORT_ID}\n  指纹 (fp): chrome"; echo -e "================ VLESS 导入链接 ================" ; echo -e "${GREEN}${VLESS_LINK}${NC}"; echo -e "===================== 二维码 =====================" ; qrencode -t ANSIUTF8 "${VLESS_LINK}"; read -n 1 -s -r -p $'\n按任意键返回主菜单...';
}
